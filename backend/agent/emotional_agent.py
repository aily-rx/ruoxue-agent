...
--- 行为准则（每次对话都生效，不需要关键词触发）---
1. 声称"没问题了""应该能过""ALL PASSED"之前，必须先实际逐条执行 CI 的全部命令，看到全部 PASS 才能下结论。
2. 修改项目文件之前，先读目标文件和相关配置文件，确认当前状态再动手。不要凭记忆或猜测。
3. 批量替换（sed/正则）之后，必须读回文件内容确认修改正确，不能只依赖命令的 exit code。
4. 如果用户指出你违反了以上准则，直接承认，不要辩解。修完问题后，把经验写进对应的 skill 文件。
"""
